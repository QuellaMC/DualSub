...
document.addEventListener('DOMContentLoaded', function () {
    ...
    (async () => {
        try {
            ...
            popupLogger.info('Popup logger initialized', {
                level: loggingLevel,
            });
        } catch (error) {
    ...
    // Consolidated slider value formatting function
    ...
    const loadSettings = async () => {
        try {
            ...
            subtitleFontSizeValue.textContent = formatSliderValue(fontSize);
            updateSliderProgress(subtitleFontSizeInput);

            const gap = settings.subtitleGap;
            subtitleGapInput.value = gap;
            subtitleGapValue.textContent = formatSliderValue(gap);
    ...
    enableSubtitlesToggle.addEventListener('change', async function () {
        try {
            ...
            popupLogger.error('Error toggling subtitles', error, {
    ...
    originalLanguageSelect.addEventListener('change', async function () {
        try {
            ...
            popupLogger.error('Error setting original language', error, {
                lang: this.value,
    ...
    targetLanguageSelect.addEventListener('change', async function () {
        try {
            ...
            popupLogger.error('Error setting target language', error, {
                lang: this.value,
    ...
    subtitleTimeOffsetInput.addEventListener('change', async function () {
        try {
            ...
            popupLogger.error('Error setting time offset', error, {
    ...
    subtitleLayoutOrderSelect.addEventListener('change', async function () {
        try {
            ...
            popupLogger.error('Error setting layout order', error, {
                layoutOrder: this.value,
    ...
    subtitleLayoutOrientationSelect.addEventListener(
        ...
        async function () {
            try {
                ...
                popupLogger.error('Error setting layout orientation', error, {
                    layoutOrientation: this.value,
    ...
    subtitleFontSizeInput.addEventListener('input', function () {
        subtitleFontSizeValue.textContent = formatSliderValue(this.value);
    ...
    subtitleFontSizeInput.addEventListener('change', async function () {
        try {
            ...
            showStatus(`${statusPrefix}${formatSliderValue(fontSize)}vw.`);

            // Send immediate update for instant visual feedback (redundant but ensures persistence)
            sendImmediateConfigUpdate({ subtitleFontSize: fontSize });
        } catch (error) {
            popupLogger.error('Error setting font size', error, {
    ...
    subtitleGapInput.addEventListener('input', function () {
        subtitleGapValue.textContent = formatSliderValue(this.value);
        updateSliderProgress(this);
    ...
    subtitleGapInput.addEventListener('change', async function () {
        try {
            ...
            const statusPrefix =
                ...
                'Vertical gap: ';
            showStatus(`${statusPrefix}${formatSliderValue(gap)}em.`);

            // Send immediate update for instant visual feedback
            sendImmediateConfigUpdate({ subtitleGap: gap });
        } catch (error) {
            popupLogger.error('Error setting subtitle gap', error, {
                gap: parseFloat(this.value),
