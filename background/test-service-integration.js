...
async function testTranslationService() {
    ...
    try {
        ...
        console.log(
            `   - Available providers: ${Object.keys(availableProviders).length}`
        );
        console.log(
            `   - Batch capable providers: ${Object.keys(batchCapable).length}`
        );
        console.log(
            `   - Rate limiting: ${rateLimitStatus.hasLimit ? 'enabled' : 'disabled'}`
        );
...
async function testSubtitleService() {
    ...
    try {
        ...
        console.log(
            `   - Available languages for sample: ${availableLanguages.length}`
        );
...
async function testServiceRegistry() {
    ...
    try {
        ...
        const translationDepsValid =
            serviceRegistry.validateDependencies('translation');
        const subtitleDepsValid =
            serviceRegistry.validateDependencies('subtitle');
        ...
        if (
            !serviceNames.includes('translation') ||
            !serviceNames.includes('subtitle')
        ) {
...
async function testServiceProtocol() {
    ...
    try {
        ...
        const successResponse = ServiceProtocol.createResponse(request, {
            result: 'Hola',
        });
        ...
        const errorResponse = ServiceProtocol.createResponse(
            request,
            null,
            error
        );
