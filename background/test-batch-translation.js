...
async function testBatchTranslationPerformance() {
    ...
    try {
        ...
        const smallBatchResults = await testBatchVsIndividual(
            sampleSubtitles,
            'en',
            'es'
        );
        ...
        const largeBatchResults = await testBatchVsIndividual(
            largeBatchSubtitles,
            'en',
            'zh-CN'
        );
        ...
        console.log('\n🎯 Performance Summary:');
        console.log(
            `Small Batch API Reduction: ${smallBatchResults.apiReduction}%`
        );
        console.log(
            `Large Batch API Reduction: ${largeBatchResults.apiReduction}%`
        );
        console.log(
            `Small Batch Time Improvement: ${smallBatchResults.timeImprovement}%`
        );
        console.log(
            `Large Batch Time Improvement: ${largeBatchResults.timeImprovement}%`
        );

        const overallApiReduction =
            (smallBatchResults.apiReduction + largeBatchResults.apiReduction) /
            2;
        const overallTimeImprovement =
            (smallBatchResults.timeImprovement +
                largeBatchResults.timeImprovement) /
            2;

        console.log(`\n📈 Overall Results:`);
        console.log(
            `Average API Call Reduction: ${overallApiReduction.toFixed(1)}%`
        );
        console.log(
            `Average Time Improvement: ${overallTimeImprovement.toFixed(1)}%`
        );
        ...
        const timeImprovementMet =
            overallTimeImprovement >= timeImprovementTarget;

        console.log(`\n✅ Performance Targets:`);
        console.log(
            `API Reduction Target (${apiReductionTarget}%): ${apiReductionMet ? '✅ MET' : '❌ NOT MET'}`
        );
        console.log(
            `Time Improvement Target (${timeImprovementTarget}%): ${timeImprovementMet ? '✅ MET' : '❌ NOT MET'}`
        );
        ...
        console.error(
            '❌ Batch translation performance test failed:',
            error.message
        );
...
async function testBatchVsIndividual(texts, sourceLang, targetLang) {
    ...
    for (const text of texts) {
        try {
            const translated = await translationProviders.translate(
                text,
                sourceLang,
                targetLang
            );
    ...
    try {
        batchResults = await translationProviders.translateBatch(
            texts,
            sourceLang,
            targetLang
        );
    ...
    const apiReduction =
        ((individualApiCalls - batchApiCalls) / individualApiCalls) * 100;
    const timeImprovement =
        ((individualTime - batchTime) / individualTime) * 100;
    ...
    console.log(`📊 Results:`);
    console.log(
        `  Individual: ${individualTime}ms, ${individualApiCalls} API calls`
    );
...
async function testBatchQueue() {
    ...
    try {
        ...
        console.log(
            `  Average Batch Size: ${metrics.averageBatchSize.toFixed(1)}`
        );
        console.log(
            `  API Call Reduction: ${metrics.apiCallReductionPercentage.toFixed(1)}%`
        );
...
async function testRateLimitingAndFallback() {
    ...
    try {
        ...
        const fallbackResults =
            await translationProviders.translateIndividually(
                fallbackTexts,
                'en',
                'fr',
                { individualDelay: 50 }
        ...
        console.log(
            `  Success: ${fallbackResults.length === fallbackTexts.length}`
        );
...
async function testProviderBatchSupport() {
    ...
    try {
        ...
        console.log(
            `  Current Provider: ${currentProvider?.name || 'Unknown'}`
        );
        console.log(`  Current Supports Batch: ${currentSupportsBatch}`);
        console.log(
            `  Batch-Capable Providers: ${Object.keys(batchProviders).length}`
        );
...
async function testBatchTranslationEmptyInput() {
    ...
    try {
        const result = await batchTranslationQueue.translateBatch(
            [],
            'en',
            'es'
        );
...
async function testBatchTranslationPartialFailure() {
    ...
    try {
        ...
        if (Array.isArray(result) && result.length === 3) {
            ...
            console.error(
                '❌ Failed: Partial failure not handled as expected.',
                result
            );
        }
    } catch (err) {
...
async function testBatchTranslationProviderTimeout() {
    ...
    try {
        ...
        if (Array.isArray(result)) {
            ...
            console.error(
                '❌ Failed: Timeout not handled as expected.',
                result
            );
        }
    } catch (err) {
...
export {
    ...
    testBatchTranslationEmptyInput,
    testBatchTranslationPartialFailure,
    testBatchTranslationProviderTimeout,
};
