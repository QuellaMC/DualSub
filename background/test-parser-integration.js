...
async function testVTTParser() {
    ...
    try {
        ...
        if (seconds !== 90.5) {
            throw new Error(
                `Timestamp parsing failed: expected 90.5, got ${seconds}`
            );
        }
...
async function testNetflixParser() {
    ...
    try {
        ...
        if (availableLanguages.length !== 2) {
            throw new Error(
                `Expected 2 available languages, got ${availableLanguages.length}`
            );
        }
        ...
        const bestTrack = netflixParser.getBestTrackForLanguage(
            validTracks,
            'en-US'
        );
        ...
        console.log(
            `   - Found ${availableLanguages.length} available languages`
        );
