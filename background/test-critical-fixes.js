...
async function testServiceWorkerCompatibility() {
    ...
    try {
        ...
        const formatted = sharedUtilityIntegration.formatSubtitleText('<b>Hello &amp; world</b>');
        ...
        console.error('❌ ServiceWorker compatibility test failed:', error.message);
        return false;
    }
}
...
async function testUniversalBatchProcessor() {
    ...
    try {
        ...
        if (!allConfigs.openai_compatible || !allConfigs.google || !allConfigs.deepl) {
        ...
        const openaiSize = universalBatchProcessor.getEffectiveBatchSize('openai_compatible');
        const googleSize = universalBatchProcessor.getEffectiveBatchSize('google');
        const deeplSize = universalBatchProcessor.getEffectiveBatchSize('deepl');
        
        if (openaiSize <= 0 || googleSize <= 0 || deeplSize <= 0) {
            throw new Error('Invalid batch sizes calculated');
        }
        
        console.log(`✅ Batch sizes: OpenAI=${openaiSize}, Google=${googleSize}, DeepL=${deeplSize}`);

        // Test batch preprocessing
        console.log('📊 Testing batch preprocessing...');
        const preprocessResult = universalBatchProcessor.preprocessForBatch(testTexts, 'openai_compatible');
        
        if (!preprocessResult.batches || preprocessResult.batches.length === 0) {
            throw new Error('Batch preprocessing failed');
        }
        
        console.log(`✅ Batch preprocessing: ${preprocessResult.batches.length} batches created`);

        // Test batch support detection
        console.log('📊 Testing batch support detection...');
        const openaiSupports = universalBatchProcessor.providerSupportsBatch('openai_compatible');
        const googleSupports = universalBatchProcessor.providerSupportsBatch('google');
        ...
        if (googleSupports) {
            throw new Error('Google should not support native batch processing');
        }
        ...
        console.error('❌ Universal batch processor test failed:', error.message);
        return false;
    }
}
...
async function testTranslationServiceIntegration() {
    ...
    try {
        ...
        if (typeof translationProviders.translateUniversalBatch !== 'function') {
        ...
        const currentSupports = translationProviders.currentProviderSupportsBatch();
        
        if (typeof currentSupports !== 'boolean') {
            throw new Error('Batch capability detection not working');
        }
        
        console.log(`✅ Batch capability: ${Object.keys(batchCapable).length} providers support batch`);
        ...
        console.error('❌ Translation service integration test failed:', error.message);
        return false;
    }
}
...
async function testConfigurationIntegration() {
    ...
    try {
        ...
        const globalBatchSize = await configService.get('globalBatchSize') || 5;
        const batchingEnabled = await configService.get('batchingEnabled') !== false;
        const useProviderDefaults = await configService.get('useProviderDefaults') !== false;
...
async function runCriticalFixesTest() {
    ...
    const results = {
        ...
        translationServiceIntegration: await testTranslationServiceIntegration(),
    ...
    console.log(`ServiceWorker Compatibility: ${results.serviceWorkerCompatibility ? '✅ PASS' : '❌ FAIL'}`);
    console.log(`Universal Batch Processor: ${results.universalBatchProcessor ? '✅ PASS' : '❌ FAIL'}`);
    console.log(`Translation Service Integration: ${results.translationServiceIntegration ? '✅ PASS' : '❌ FAIL'}`);
    console.log(`Configuration Integration: ${results.configurationIntegration ? '✅ PASS' : '❌ FAIL'}`);
