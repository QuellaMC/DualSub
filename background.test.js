...
describe('Background Script Logger Integration', () => {
    ...
    test('should log error messages with stack traces', () => {
        ...
        backgroundLogger.error('Test error message', testError, { context: 'test' });
    ...
    test('should update logging level from config service', async () => {
        const mockGet = jest.spyOn(configService, 'get').mockResolvedValue(Logger.LEVELS.DEBUG);
    ...
    test('should handle config service errors gracefully', async () => {
        const mockGet = jest.spyOn(configService, 'get').mockRejectedValue(new Error('Config error'));
    ...
    test('should format messages with timestamp and component', () => {
        const message = backgroundLogger.formatMessage('INFO', 'Test message', { data: 'test' });
