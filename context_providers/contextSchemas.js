...
const culturalSchema = {
  ...
  required: ['definition', 'cultural_context', 'usage', 'cultural_significance', 'learning_tips', 'related_expressions', 'sensitivities'],
  additionalProperties: false
};
...
const historicalSchema = {
  ...
  required: ['definition', 'historical_context', 'evolution', 'historical_significance', 'examples', 'related_terms', 'learning_context'],
  additionalProperties: false
};
...
const linguisticSchema = {
  ...
  required: ['definition', 'etymology', 'grammar', 'semantics', 'translation_notes', 'examples', 'related_forms', 'learning_tips'],
  additionalProperties: false
};
...
const allSchema = {
  ...
  required: ['definition', 'cultural_analysis', 'historical_analysis', 'linguistic_analysis', 'practical_usage', 'learning_tips', 'related_expressions', 'key_insights'],
  additionalProperties: false
};
...
export function validateAgainstSchema(schema, data) {
  function validate(node, value) {
    if (node.type === 'object') {
      ...
      if (node.properties) {
        for (const [key, schemaChild] of Object.entries(node.properties)) {
          if (key in value && !validate(schemaChild, value[key])) return false;
        }
      }
