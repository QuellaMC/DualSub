...
describe('Logger Integration Tests', () => {
    ...
    describe('Logging Level Changes Across All Contexts', () => {
        ...
        it('should handle real-time logging level updates via message passing', async () => {
            const backgroundLogger = Logger.create('Background', mockConfigService);
            const contentLogger = Logger.create('ContentScript');
            
            // Setup message listener simulation
            let messageListener = null;
            mockChromeStorage.runtime.onMessage.addListener.mockImplementation((listener) => {
    ...
    describe('Message Passing System for Real-time Updates', () => {
        ...
        it('should handle message passing failures gracefully', async () => {
            ...
            mockChromeStorage.tabs.sendMessage.mockRejectedValue(new Error('Tab not found'));
        ...
        it('should handle content script message reception', () => {
            ...
            mockChromeStorage.runtime.onMessage.addListener.mockImplementation((handler) => {
            ...
            expect(mockChromeStorage.runtime.onMessage.addListener).toHaveBeenCalled();
            
            // Get the registered handler
            const registeredHandler = mockChromeStorage.runtime.onMessage.addListener.mock.calls[0][0];
    ...
    describe('Performance Impact Validation', () => {
        it('should have minimal performance impact when logging is disabled', async () => {
            ...
            for (let i = 0; i < iterations; i++) {
                ...
                logger.error('Error message', null, { iteration: i, data: 'test' });
            }
    ...
    describe('Extension Restart and Configuration Persistence', () => {
        it('should maintain logging configuration across simulated extension restarts', async () => {
            ...
            const session1Logger = Logger.create('Background', mockConfigService);
            ...
            const session2Logger = Logger.create('Background', mockConfigService);
            ...
            expect(consoleSpies.debug).toHaveBeenCalledWith(
                expect.stringContaining('[DEBUG] [Background] Debug after restart')
            );
        });
        ...
        it('should handle configuration corruption gracefully on restart', async () => {
            ...
            expect(consoleSpies.info).toHaveBeenCalledWith(
                expect.stringContaining('[INFO] [Background] Info after fixing level')
            );
        });
        ...
        it('should handle storage quota exceeded scenarios', async () => {
            ...
            const logger = Logger.create('Background', quotaExceededConfigService);
            await logger.updateLevel();
            ...
            expect(consoleSpies.warn).toHaveBeenCalledWith(
                expect.stringContaining('[WARN] [Background] Warning with quota exceeded')
            );
        });
    });
    ...
    describe('Real-world Integration Scenarios', () => {
        it('should handle video platform logging integration', async () => {
            ...
            const netflixLogger = Logger.create('NetflixPlatform', mockConfigService);
            await netflixLogger.updateLevel();
            
            // Simulate Disney+ platform logger
            const disneyLogger = Logger.create('DisneyPlusPlatform', mockConfigService);
            ...
            expect(consoleSpies.info).toHaveBeenCalledWith(
                expect.stringContaining('[INFO] [NetflixPlatform] Subtitle data received')
            );
            ...
            expect(consoleSpies.debug).toHaveBeenCalledWith(
                expect.stringContaining('[DEBUG] [DisneyPlusPlatform] Video context changed again')
            );
        });

        it('should handle translation provider logging integration', async () => {
            // Simulate different translation providers
            const googleLogger = Logger.create('GoogleTranslate', mockConfigService);
            const deeplLogger = Logger.create('DeepLTranslate', mockConfigService);
            const microsoftLogger = Logger.create('MicrosoftTranslate', mockConfigService);
            ...
            microsoftLogger.error('Translation failed', new Error('Network timeout'), { 
                retryCount: 3 
            });
            
            expect(consoleSpies.info).toHaveBeenCalledWith(
                expect.stringContaining('[INFO] [GoogleTranslate] Translation request')
            );
            expect(consoleSpies.warn).toHaveBeenCalledWith(
                expect.stringContaining('[WARN] [DeepLTranslate] API rate limit approaching')
            );
            expect(consoleSpies.error).toHaveBeenCalledWith(
                expect.stringContaining('[ERROR] [MicrosoftTranslate] Translation failed')
            );
        });
        ...
        it('should handle service class logging integration', async () => {
            ...
            const errorHandlerLogger = Logger.create('ConfigServiceErrorHandler', mockConfigService);
            ...
            errorHandlerLogger.error('Storage operation failed', new Error('Chrome API error'), { 
                operation: 'get',
                keys: ['loggingLevel']
            });
            ...
            // Error should log
            expect(consoleSpies.error).toHaveBeenCalledWith(
                expect.stringContaining('[ERROR] [ConfigServiceErrorHandler] Storage operation failed')
            );
        });
        ...
        it('should handle content script logging with fallback mechanisms', async () => {
            ...
            expect(consoleSpies.info).toHaveBeenCalledWith(
                expect.stringContaining('[INFO] [ContentScript] Content script initialized')
            );
            expect(consoleSpies.debug).not.toHaveBeenCalled();
            expect(consoleSpies.warn).toHaveBeenCalledWith(
                expect.stringContaining('[WARN] [ContentScript] Subtitle injection warning')
            );
            ...
            expect(consoleSpies.debug).toHaveBeenCalledWith(
                expect.stringContaining('[DEBUG] [ContentScript] Debug after level update')
            );
        });
    });
    ...
    describe('Stress Testing and Edge Cases', () => {
        ...
        it('should handle concurrent logging from multiple components', async () => {
            ...
            const loggingPromises = loggers.map(async (logger, index) => {
                for (let i = 0; i < 10; i++) {
                    ...
                    if (i % 3 === 0) {
                        logger.error(`Error ${i} from component ${index}`, new Error(`Test error ${i}`));
                    }
        ...
        it('should handle invalid logging level values gracefully', async () => {
            ...
            const directSetValues = [
                -1, 5, 'debug', {}, [], true, false
            ];
            ...
            for (const invalidValue of directSetValues) {
                ...
                if (typeof invalidValue === 'number' && invalidValue >= Logger.LEVELS.INFO) {
