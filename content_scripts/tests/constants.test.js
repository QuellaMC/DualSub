...
describe('Content Script Constants', () => {
    describe('COMMON_CONSTANTS', () => {
        ...
        test('should have UI-only settings array', () => {
            ...
            expect(COMMON_CONSTANTS.UI_ONLY_SETTINGS).toContain(
    ...
    describe('PLATFORM_CONSTANTS', () => {
        test('should have Netflix constants', () => {
            ...
            expect(netflix.INJECT_SCRIPT_FILENAME).toBe(
                'injected_scripts/netflixInject.js'
            );
            expect(netflix.INJECT_SCRIPT_TAG_ID).toBe(
                'netflix-dualsub-injector-script-tag'
            );
            expect(netflix.INJECT_EVENT_ID).toBe(
        ...
        test('should have Disney+ constants', () => {
            ...
            expect(disneyplus.INJECT_SCRIPT_FILENAME).toBe(
                'injected_scripts/disneyPlusInject.js'
            );
            expect(disneyplus.INJECT_SCRIPT_TAG_ID).toBe(
                'disneyplus-dualsub-injector-script-tag'
            );
            expect(disneyplus.INJECT_EVENT_ID).toBe(
    ...
    describe('DEFAULT_PLATFORM_CONFIGS', () => {
        test('should have valid Netflix configuration', () => {
            ...
            expect(netflix.injectScript.filename).toBe(
                PLATFORM_CONSTANTS.netflix.INJECT_SCRIPT_FILENAME
            );
            expect(netflix.navigation.spaHandling).toBe(true);
            expect(netflix.videoDetection.maxRetries).toBe(
        ...
        test('should have valid Disney+ configuration', () => {
            ...
            expect(disneyplus.injectScript.filename).toBe(
                PLATFORM_CONSTANTS.disneyplus.INJECT_SCRIPT_FILENAME
            );
            expect(disneyplus.navigation.spaHandling).toBe(false);
            expect(disneyplus.videoDetection.maxRetries).toBe(
                COMMON_CONSTANTS.MAX_VIDEO_DETECTION_RETRIES
            );
        });
