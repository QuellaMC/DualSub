...
JSON.parse = function (text, reviver) {
    // console.log("[Inject DBG] JSON.parse called. Length:", text.length); // Very noisy, use for deep debug
    let parsedObject;
    try {
        parsedObject = originalJSONParse(text, reviver); // Pass reviver as original does
    } catch (e) {
        // console.error("[Inject v6] Error in originalJSONParse:", e, "Text:", text.substring(0,500));
        throw e; // Re-throw error so the page can handle it if it's not the JSON we care about
    }

    try {
        // Path for Disney+ subtitle master playlist URL (M3U8)
        // Example path: data.stream.sources[0].complete.url (from initial analysis)
        // More general path might be needed if structure varies.
        // This specific path was from previous successful extraction.
        if (parsedObject &&
            parsedObject.data && // Often data is nested under a 'data' key
            ...
            // Example: https://www.disneyplus.com/video/XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX
            ...
            if (videoIndex !== -1 && videoIndex < pathSegments.length - 1) {
                ...
                // Basic validation for a UUID-like structure, adjust if Disney's ID format is different
        ...
        // For example, sometimes it might be directly in parsedObject.stream...
        ...
        // Don't log error for every JSON.parse, only if it's one we might care about or for deep debugging
