...
class LoggerMock {
  ...
  getLogsByMessage(messageText) {
    return this.logs.filter(log => 
      log.message && log.message.includes(messageText)
    );
  }
  ...
  wasLogged(level, message) {
    return this.logs.some(log => 
      log.level === level && log.message === message
    );
  }
    ...
    return {
      ...
      toHaveLoggedCount: (count) => {
        ...
        return {
          ...
          message: () => `Expected ${count} log messages, but got ${actualCount}`
        };
      },
      ...
      toHaveNoErrors: () => {
        ...
        return {
          ...
          message: () => `Expected no errors, but found ${errors.length}: ${JSON.stringify(errors)}`
        };
      }
    };
  }
