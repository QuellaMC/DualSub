...
class TestHelpers {
    ...
    createDisneyPlusEvent(type, options = {}) {
        const { videoId = 'abc123', url = 'http://example.com/master.m3u8' } =
            options;
    ...
    get assertions() {
        return {
            ...
            expectStorageAccessed: (
                ...
                if (expectedArgs) {
                    expect(
                        chromeApiMock.storage.sync[method]
                    ).toHaveBeenCalledWith(expectedArgs, expect.any(Function));
                } else {
                    expect(
                        chromeApiMock.storage.sync[method]
                    ).toHaveBeenCalled();
             ...
             */
            expectRuntimeMessageSent: (
                chromeApiMock,
                expectedMessage = null
            ) => {
                if (expectedMessage) {
                    expect(
                        chromeApiMock.runtime.sendMessage
...
class PlatformTestSuiteGenerator {
        ...
        return function () {
            ...
            describe('Event Handling', () => {
                test('should handle inject script ready event', () => {
                    const readyEvent = testHelpers.createMockEvent(
                        config.eventTypes.ready
                    );
    ...
    static generateNetflixTestSuite(NetflixPlatformClass) {
        ...
        const baseSuite = this.generateTestSuite(
            NetflixPlatformClass,
            baseConfig
        );
        ...
        return function () {
            ...
            describe('Netflix-Specific Tests', () => {
                test('should handle subtitle data found event', () => {
                    const subtitleEvent =
                        testHelpers.createNetflixEvent('subtitleData');
        ...
        return function () {
            ...
            describe('Disney Plus-Specific Tests', () => {
                test('should handle subtitle URL found event', () => {
                    const subtitleEvent =
                        testHelpers.createDisneyPlusEvent('subtitleUrl');
