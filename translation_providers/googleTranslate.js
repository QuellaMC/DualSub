...
export async function translate(text, sourceLang, targetLang) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            console.error(
                `GoogleTranslate: API HTTP error! Status: ${response.status}, Response: ${errorText.substring(0, 200)}`
            );
        ...
        if (contentType && contentType.includes('application/json')) {
            ...
            if (
                ...
                console.error(
                    'GoogleTranslate: Translation JSON parsing failed or unexpected structure. Response data:',
                    data
                );
                throw new Error(
            ...
            console.error(
                `GoogleTranslate: API did not return JSON. Content-Type: ${contentType}. Response (first 500 chars):`,
