...
if (window.netflixDualSubInjectorLoaded) {
    console.log("Netflix Inject script: Already loaded, skipping initialization.");
} else {
    ...
    console.log("Netflix Inject script: Overriding JSON.parse to intercept subtitle data.");
    ...
    JSON.parse = function (text, reviver) {
        ...
        try {
            ...
            if (parsedObject && parsedObject.result && parsedObject.result.timedtexttracks && parsedObject.result.movieId) {
                console.log(`%c[Netflix Inject] Found Netflix subtitle data for movieId: %s`, "color: blue; font-weight: bold;", parsedObject.result.movieId);
                
                // Dispatch event using the same pattern as Disney+
                document.dispatchEvent(new CustomEvent(INJECT_EVENT_ID, {
                    detail: {
                        type: 'SUBTITLE_DATA_FOUND',
                        payload: {
                            movieId: parsedObject.result.movieId,
                            timedtexttracks: parsedObject.result.timedtexttracks
                        }
                    }
                }));
                console.log("[Netflix Inject] Dispatched SUBTITLE_DATA_FOUND event.");
            }
        } catch (e) {
    ...
    document.dispatchEvent(new CustomEvent(INJECT_EVENT_ID, { detail: { type: 'INJECT_SCRIPT_READY' } }));
